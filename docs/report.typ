...
我们在不同模式与误码率下进行压力测试；在实现完整 GBN 逻辑后，程序能在默认误码与高误码场景下持续运行而不出现网络层「坏分组」中止（指导书 8.13）。下表为 2026 年 5 月 15 日实测 600 秒运行末段 #raw("lprintf") 统计数据。
...
#align(center)[
  #table(
    ...
    [1], [无误码数据传输], [#raw("-u -d0 -t 600")], [39.36%], [69.64%],
    [2], [默认平缓业务], [#raw("-d0 -t 600")], [34.18%], [61.81%],
    [3], [双端洪水 + 无误码], [#raw("-f -u -d0 -t 600")], [72.32%], [72.30%],
    [4], [双端洪水 + 默认误码], [#raw("-f -d0 -t 600")], [63.69%], [62.80%],
    [5], [洪水 + 高误码 $(10^(-4))$], [#raw("-f -b 1e-4 -d0 -t 600")], [32.02%], [31.24%],
  )
]

#emph[（注：五个场景均自然 Quit，日志中未出现 bad packet / Abort；完整命令、端口与日志文件见 `docs/实验报告.typ` 附录。）]
...
GBN 要保持「管道不空」，窗口内未确认数据在途时间应覆盖一次往返：粗略有 $W * T_x >= T_x + "RTT"$，代入得 $W >= (T_x + "RTT") / T_x approx 810 / 263 approx 3.08$。本组按字节 BDP 折中取 #raw("WINDOW_SIZE=3")。序号字段为 1 字节，最终使用完整 #raw("MAX_SEQ=255")（0–255）空间，避免误码重传时旧帧在物理层队列中滞留、序号过快回绕后被接收端误收。

重传定时器 #raw("DATA_TIMEOUT_MS=600") 略大于传播往返 540 ms；库函数 #raw("start_timer") 还会自动叠加 #raw("phl_sq_len") 折算出的本地物理层排队发送时间，减少帧尚未离站就被误判超时的情况。

#strong[2. 利用率上界与实测对比]

无误码且载荷占主导时，线路利用率上界近似为数据字节占帧长比例：$256 / 263 approx 97.34%$。表中情景 3 的 72.3% 低于理想上界，主要受 #raw("WINDOW_SIZE=3")、ACK 等待、事件调度以及物理层排队影响；但两端数值接近，说明全双工洪水下窗口能稳定滑动。
...
- #strong[序号空间调试经历]：
  早期 #raw("MAX_SEQ=7") 版本在默认误码长跑中曾出现网络层坏分组。原因是旧重传帧可能在物理层队列中滞留到接收端序号绕回后被误收。最终将 1 字节序号空间完整用于编号，改为 #raw("MAX_SEQ=255") / #raw("NR_BUFS=256")，窗口仍为 3；复跑五个表 3 场景各 600 秒均无坏分组。
- #strong[总结]：
